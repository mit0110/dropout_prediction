...
def transform_input(train_sequences, test_sequences):
    ...
    """
    return (
        numpy.array([numpy.array(x)[:,0] for x in train_sequences]),
        numpy.array([numpy.array(x)[:,0] for x in test_sequences])
    )
...
def main():
    ...
    for period, train_sequences, test_sequences in get_periods(args):
        ...
        train_instances, test_instances = transform_input(
            train_sequences.sequence.values, test_sequences.sequence.values)
        kddcup_dataset = KDDCupDataset(padding_value=-1)
        kddcup_dataset.create_fixed_samples(
            train_instances, train_sequences.dropout.values,
            test_instances, test_sequences.dropout.values,
